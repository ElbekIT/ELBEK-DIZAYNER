  ...
  return (
    <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto min-h-screen">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-zinc-800 font-black uppercase">No items found</div>
      ) : (
...
const PortfolioManagerAdmin = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(pRef, snapshot => {
      const data = snapshot.val();
      setItems(data ? (Object.values(data) as PortfolioItem[]) : []);
    });
    return () => off(pRef, 'value', unsub);
  }, []);
...
const UserModerationAdmin = () => {
  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto">
      {users.length === 0 ? <p className="text-center text-zinc-800 font-black uppercase">No users found</p> : users.map(u => (
